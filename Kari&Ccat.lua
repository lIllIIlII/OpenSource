--!nocheck
-- bc i need to use luau lsp to check syntax error,made by karisob,discord name:"333333T",qq:"3521551513" ccat,discord name :"Ccat",qq:"3395858053"
local bit32_band = bit32.band
local bit32_rshift = bit32.rshift

local LBC_VERSION_MIN = 3
local LBC_VERSION_MAX = 12
...
local LBC_CONSTANT_VECTORD = 10
local LBC_CONSTANT_CLASS_SHAPE = 11
...
local function parseProto(r, bc)
	local p = newProto()
	-- v12+: each proto is prefixed with its byte size
	if bc.version >= 12 then
		p.protoSize = r:varint()
		r._protoStart = r.pos
	end
	p.maxstacksize = r:u8()
	p.numparams = r:u8()
	...
	for _ = 1, sizek do
		...
		elseif ktype == LBC_CONSTANT_VECTORD then
			push(p.constants, newConstant("vectord", { r:f64(), r:f64(), r:f64(), r:f64() }))
		elseif ktype == LBC_CONSTANT_CLASS_SHAPE then
			local cnid = r:varint()
			local numProperties = r:varint()
			local numMethods = r:varint()
			local members = {}
			for _ = 1, numProperties + numMethods do
				push(members, r:varint())
			end
			push(p.constants, newConstant("class_shape", { cnid = cnid, numProperties = numProperties, numMethods = numMethods, members = members }))
		else
	...
	-- v11+: feedback vector slots
	if bc.version >= 11 then
		local fbsz = r:varint()
		p.feedbackvec = {}
		for _ = 1, fbsz do
			local slottype = r:u8()
			local pc = r:varint()
			push(p.feedbackvec, { type = slottype, pc = pc })
		end
	end

	-- v12+: cost (if inlinable flag set, bit 0 of flags)
	if bc.version >= 12 then
		if bit32_band(p.flags, 1) ~= 0 then
			p.cost = r:varint64()
		end
	end

	-- v12+: skip remaining bytes using protoSize
	if bc.version >= 12 and p.protoSize then
		local readSoFar = r.pos - r._protoStart
		local remaining = p.protoSize - readSoFar
		if remaining > 0 then
			r:bytes(remaining)
		end
	end
