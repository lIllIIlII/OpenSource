--[[
修复忍者反编译问题 反编译模块来自Karisob&Ccat
decompile Module By Karisob & Ccat
fixed delta decompile v12
]]
...
Main = (function()
	...
	Main.InitEnv = function()
		...
		env.decompile = (function()
			-- Patched: use the GitHub-hosted Luau decompiler (noahfr / By Karisob)
			-- instead of the injector's `decompile` global or the Konstant HTTP API.
			-- The GitHub library is a pure Luau module: it takes raw bytecode and
			-- returns decompiled source. We still need getscriptbytecode() to read
			-- the bytecode from a Roblox script (there is no other way), but the
			-- decompilation itself is done locally without any injector decompiler.

			if not env.getscriptbytecode then
				warn("[Dex++] Cannot enable GitHub decompiler: getscriptbytecode is missing.")
				return nil
			end

			local NOAHFR_URL = "https://raw.githubusercontent.com/lIllIIlII/OpenSource/refs/heads/main/Kari%26Ccat.lua"

			-- Fetch the decompiler source from GitHub.
			local fetchOk, decompilerSrc = pcall(function()
				return oldgame:HttpGet(NOAHFR_URL, true)
			end)
			if not fetchOk or type(decompilerSrc) ~= "string" or decompilerSrc == "" then
				warn(("[Dex++] Failed to fetch GitHub decompiler from %s: %s")
					:format(NOAHFR_URL, tostring(decompilerSrc)))
				return nil
			end

			-- Compile the source as a Lua module.
			local loadOk, loadFn = pcall(loadstring, decompilerSrc, "noahfr")
			if not loadOk or type(loadFn) ~= "function" then
				warn(("[Dex++] Failed to compile GitHub decompiler: %s"):format(tostring(loadFn)))
				return nil
			end

			-- Run it once to get the module table.
			local runOk, Module = pcall(loadFn)
			if not runOk or type(Module) ~= "table" or type(Module.decompile) ~= "function" then
				warn(("[Dex++] GitHub decompiler did not return a valid module: %s"):format(tostring(Module)))
				return nil
			end

			-- Cache the module so we don't re-fetch on every call.
			local NoahfrModule = Module

			local function decompile(scriptPath)
				-- Step 1: read raw bytecode from the Roblox script.
				local bcOk, bytecode = pcall(env.getscriptbytecode, scriptPath)
				if not bcOk or type(bytecode) ~= "string" or #bytecode == 0 then
					return ("-- Failed to get script bytecode.\n--[[\n%s\n--]]"):format(tostring(bytecode))
				end

				-- Step 2: run the GitHub decompiler locally.
				local ok, source = pcall(NoahfrModule.decompile, bytecode, { mode = "source" })
				if not ok or type(source) ~= "string" then
					return ("-- GitHub decompiler raised an error:\n--[[\n%s\n--]]"):format(tostring(source))
				end
				return source
			end
